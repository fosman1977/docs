...
- Barrister utilisation (30 minute TTL)
...
| `utilisation:{id}` | `utilisation:user123` | Barrister utilisation |
...
async function updateBarristerProfile(barristerId: string, data: any) {
  ...
  await cacheDelete(`utilisation:${barristerId}`);
}
```
...
| Utilisation | 30 minutes | Workload data updates |
