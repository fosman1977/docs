...
### clerk_todo_streaks Table

Tracks completion streaks for gamification.

```sql
CREATE TABLE clerk_todo_streaks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
```typescript
const result = await todoService.getStreak('user-uuid');
```

#### getStreakLeaderboard()

Get top 10 streaks across all users.

```typescript
const result = await todoService.getStreakLeaderboard();
```
...
```sql
-- Updates user streak when a todo is completed
CREATE TRIGGER trigger_update_clerk_todo_streak
    AFTER UPDATE ON clerk_todos
    FOR EACH ROW
    EXECUTE FUNCTION update_clerk_todo_streak();
```
...
### clerk_todo_streaks Policies

| Policy | Operation | Rule |
|--------|-----------|------|
| Users can view own streaks | SELECT | `user_id = auth.uid()` |
| Clerks can view all streaks | SELECT | `has_operational_access()` |
| System can manage streaks | ALL | `true` (for trigger operations) |
...
| TodoStreakBadge | `$lib/components/features/todos/TodoStreakBadge.svelte` | Streak display |
